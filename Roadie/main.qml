...
Window {
    ...
    // Outer white frame to verify scaling on device
    Rectangle {
        anchors.fill: parent
        color: "transparent"
        border.color: "white"
    ...
    Row {
        ...
        // LEFT SIDE: main panel + info tiles
        Column {
            id: leftCol
            spacing: 10
            ...
            // Forward warning detection main panel
            ForwardWarningPanel {
                id: fwdPanel
                ...
                // Example: switch modes from Python later:
                // fwdPanel.mode = "idle" | "warning" | "error" | "customX"
            }

            // Bottom info tiles row (data-driven)
            Row {
                id: infoRow
                spacing: 10
                anchors.left: parent.left
                ...
                // Model: update values later via model.set(), or replace with a ListView
                ListModel {
                    id: infoModel
        ...
        // RIGHT SIDE: sidebar with one rectangular and two round buttons
        Column {
            id: sideBar
            spacing: 10
            width: 220
            ...
            // Push the bottom circle up a bit if window is short
            Item { anchors.verticalStretch: 1 }
        }
    }

    // --- COMPONENTS ------------------------------------------------------------

    // Forward warning panel with simple mode switching, ready for more cases
    Component {
        id: forwardWarningPanelComp
    }
    component ForwardWarningPanel: Rectangle {
        ...
        // --- Built-in cases you can extend/replace with real content -----------
        Component {
            id: idleCase
            Item {
                Text {
    ...
    component RoundButton: Rectangle {
        ...
        MouseArea {
            anchors.fill: parent
            onClicked: roundBtn.clicked()
        }
    }

    // Info tile (title + value)
